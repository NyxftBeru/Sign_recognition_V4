...
class SignLanguageQuiz {
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e)=>{
            if (window.innerWidth <= 768) {
                if (!this.sidebar.contains(e.target) && !e.target.closest('.sidebar-toggle')) this.sidebar.classList.remove('open');
            }
        });
    }
    ...
    loadFirestoreCache() {
        try {
            ...
            if (data && data.timestamp && Date.now() - data.timestamp < 86400000) return data.signs;
            return null;
        } catch (error) {
    ...
    async loadQuizData() {
        try {
            ...
            if (this.firestoreCache && this.firestoreCache.length > 0) {
                ...
                if (signsCollection.empty) throw new Error('No signs found in the collection');
                this.signs = [];
                signsCollection.forEach((doc)=>{
                    const data = doc.data();
                    if (data.base64) this.signs.push({
                        id: doc.id,
                        name: doc.id,
                        base64: data.base64
                    });
                ...
                if (this.signs.length === 0) throw new Error('No valid signs with base64 data found');
    ...
    generateSidebar() {
        ...
        this.signs.forEach((sign, index)=>{
            ...
            if (index < 5) this.questionListSection1.appendChild(questionItem);
            else if (index < 10) this.questionListSection2.appendChild(questionItem);
            else this.questionListSection3.appendChild(questionItem);
        });
    }
    toggleSidebar() {
    ...
    navigateToQuestion(index) {
        if (index >= 0 && index < this.signs.length) {
            ...
            if (this.isInResultsSummary) return; // Do nothing when on results summary page
    ...
    updateSidebarHighlight() {
        ...
        allQuestionItems.forEach((item, index)=>{
            ...
            if (!this.isInResultsSummary && index === currentIndex) item.classList.add('current');
            else {
                const userAnswer = this.userAnswers[index];
                if (userAnswer) {
                    item.classList.add('answered');
                    // Only show correct/incorrect after quiz is submitted
                    if (this.isQuizSubmitted && userAnswer.isCorrect === false) item.classList.add('incorrect');
                }
            }
        });
    }
    ...
    generateAllQuestionOptions() {
        ...
        this.signs.forEach((correctSign)=>{
            ...
            for(let i = 0; i < 3 && i < wrongOptions.length; i++)options.push(wrongOptions[i]);
    ...
    async batchGenerateSentencesForSection2() {
        try {
            ...
            console.log(`\u{1F504} Generating sentences for ${missingWords.length} words in batch:`, missingWords);
...
2. explain: Please explain the concept to me.`;
            ...
            lines.forEach((line)=>{
                ...
                if (match) {
                    ...
                    console.log(`\u{1F4BE} Cached sentence for "${word}": ${sentence}`);
                }
            });
            // Save the updated cache
            this.saveOpenAICache();
            console.log(`\u{2705} Batch generated and cached sentences for ${missingWords.length} words`);
            return true;
        } catch (error) {
    ...
    async evaluateSection3Answers() {
        ...
        const section3Answers = this.userAnswers.slice(10).filter((answer)=>answer && answer.userAnswer && answer.userAnswer.trim().length > 0);
        ...
        try {
            ...
            section3Answers.forEach((answer, index)=>{
                ...
                answer.evaluation = hasValidAnswer ? "Your answer was submitted successfully. Due to technical limitations, detailed AI evaluation is currently unavailable." : "Please provide a more detailed description of the sign gesture.";
    ...
    async displayQuestion() {
        ...
        } else // Section 3 - no multiple choice options, just display text input
        this.displayOptions([]);
        ...
        if (this.currentQuestionIndex >= 10) {
            ...
            videoContainer.style.display = 'none';
            quizContainer.classList.add('center-content');
            if (this.openaiDisabled) this.updateQuestionForSection2Fallback(currentSign.name, this.currentQuestionIndex + 1);
            else this.updateQuestionForOpenAI(currentSign.name);
        } else {
        ...
        if (this.currentQuestionIndex !== 5) this.signVideo.play().catch((error)=>{
    ...
    async showOpenAISentence(word) {
        ...
        sentenceContainer.innerHTML = `
            <div class="sentence-header">
                <h3>\u{1F4DD} Example Sentence for Question 6</h3>
            </div>
        ...
        try {
            ...
            sentenceContainer.innerHTML = `
                <div class="sentence-header">
                    <h3>\u{274C} AI Sentence Generation Failed</h3>
                </div>
                <div class="sentence-content">
    ...
    async updateQuestionForOpenAI(word) {
        ...
        try {
            ...
            if (this.currentQuestionIndex >= 5 && this.currentQuestionIndex <= 9) {
                ...
                if (!batchSuccess) console.log("\uD83D\uDD04 Batch failed, falling back to individual generation");
            }
    ...
    displayOptions(options) {
        ...
        if (isSection3) {
            ...
            options.forEach((option, index)=>{
                ...
                const previousAnswer = this.userAnswers[this.currentQuestionIndex];
                if (previousAnswer && previousAnswer.userAnswer === option.name) optionDiv.classList.add('selected');
                optionDiv.innerHTML = `
            ...
            options.forEach((option, index)=>{
                ...
                // Check if this option was previously selected
                const previousAnswer = this.userAnswers[this.currentQuestionIndex];
                if (previousAnswer && previousAnswer.userAnswer === option.name) optionCard.classList.add('selected');
    ...
    selectAnswer(selectedSign, selectedButton) {
        ...
        if (this.currentQuestionIndex >= 5 && this.currentQuestionIndex < 10 && this.aiSentence) answerData.aiSentence = this.aiSentence;
    ...
    updateButtonVisibility() {
        ...
        const hasAnswer = currentAnswer !== null && (this.currentQuestionIndex < 10 || this.currentQuestionIndex >= 10 && currentAnswer.userAnswer.trim().length > 0);
        const isLastQuestion = this.currentQuestionIndex === this.signs.length - 1;
        const allAnswered = this.userAnswers.every((answer)=>answer !== null && answer.userAnswer !== null && answer.userAnswer.toString().trim().length > 0);
        // Hide replay button for Section 3 questions (11-15) since there's no video
        if (this.currentQuestionIndex >= 10) this.replayBtn.classList.add('hidden');
        else this.replayBtn.classList.remove('hidden');
        // Show next button if answered and not last question
        if (hasAnswer && !isLastQuestion) this.nextBtn.classList.remove('hidden');
        else this.nextBtn.classList.add('hidden');
        // Show submit button if all questions answered
        if (allAnswered && !this.isQuizSubmitted) this.submitBtn.classList.remove('hidden');
        else this.submitBtn.classList.add('hidden');
    }
    async submitQuiz() {
        this.isQuizSubmitted = true;
        this.score = 0;
        // Show loading message for Section 3 evaluation if needed
        const hasSection3Answers = this.userAnswers.slice(10).some((answer)=>answer && answer.userAnswer && answer.userAnswer.trim().length > 0);
        if (hasSection3Answers && !this.openaiDisabled) this.showEvaluationLoading();
        ...
        if (hasSection3Answers && !this.openaiDisabled) try {
        ...
        else if (hasSection3Answers) // OpenAI disabled - mark as pending manual review
    ...
    nextQuestion() {
        ...
        if (this.currentQuestionIndex < this.signs.length) this.displayQuestion();
    ...
    async startQuizReplayChat() {
        ...
        // Welcome message
        await this.addChatMessage('bot', `\u{1F389} Let's review your Sign Language Quiz journey together!`);
        await this.delay(1000);
        await this.addChatMessage('bot', `I'll replay each question, show your answers, and give you feedback. Ready? \u{1F680}`);
        ...
        for(let i = 0; i < this.signs.length; i++){
            ...
            if (i < this.signs.length - 1) await this.delay(800);
        }
        // Final summary
        await this.delay(1000);
    ...
    async replayQuestionAsChat(questionIndex) {
        ...
        if (questionIndex < 5) // Section 1: Text-based questions
        await this.addChatMessage('bot', `<strong>Question ${questionIndex + 1}:</strong> What sign was being demonstrated?`);
            ...
            if (answer && answer.aiSentence) {
                ...
                if (sentenceText === answer.aiSentence) {
                    ...
                    if (partialRegex.test(answer.aiSentence)) sentenceText = answer.aiSentence.replace(partialRegex, blanks);
                    else // Fallback: just add blanks at the end
        ...
        } else // Section 3: Description questions
        ...
        // User's answer
        if (answer) {
            if (questionIndex < 10) // Multiple choice answer
            await this.addChatMessage('user', `${answer.userAnswer}`);
            else {
                // Text description (show preview if too long)
                const answerText = answer.userAnswer.length > 150 ? answer.userAnswer.substring(0, 150) + '...' : answer.userAnswer;
                await this.addChatMessage('user', answerText);
            }
        } else await this.addChatMessage('user', `<em>No answer provided</em>`);
    ...
    async provideBotFeedback(questionIndex, sign, answer) {
        if (!answer) {
            await this.addChatMessage('bot', `\u{274C} You didn't answer this question. The correct answer was: <strong>"${sign.name}"</strong>`);
            return;
        }
        if (questionIndex < 10) {
            ...
            } else await this.addChatMessage('bot', `\u{274C} Not quite right. You answered "<strong>${answer.userAnswer}</strong>" but the correct answer was "<strong>${sign.name}</strong>". Keep practicing! \u{1F4AA}`);
        } else // Section 3: Description feedback
        ...
        } else await this.addChatMessage('bot', `\u{1F4DD} I wasn't able to evaluate your description due to technical limitations, but thank you for your detailed response about the "${sign.name}" sign!`);
    }
    async showFinalSummary() {
        const percentage = Math.round(this.score / this.signs.length * 100);
        await this.addChatMessage('bot', `\u{1F38A} <strong>Quiz Complete!</strong> Here's your final summary:`);
        await this.delay(1000);
        // Overall score with encouragement
        const scoreMessage = `\u{1F4CA} <strong>Final Score: ${this.score}/${this.signs.length} (${percentage}%)</strong>`;
        let encouragement = '';
        if (percentage >= 90) encouragement = "\uD83C\uDFC6 Outstanding! You're a sign language champion!";
        else if (percentage >= 80) encouragement = "\uD83C\uDF1F Excellent work! You have great sign language skills!";
        else if (percentage >= 70) encouragement = "\uD83D\uDC4F Well done! You're making great progress!";
        else if (percentage >= 60) encouragement = "\uFFFD Good effort! Keep practicing and you'll improve!";
        else encouragement = "\uD83D\uDCDA Great start! Sign language takes practice - you're on the right path!";
        await this.addChatMessage('bot', `${scoreMessage}<br><br>${encouragement}`);
        await this.delay(1000);
        // Section breakdown
        await this.addChatMessage('bot', `\u{1F4C8} <strong>Section Breakdown:</strong>`);
        ...
        const breakdownHtml = `
            <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 10px; margin: 10px 0;">
                <div style="margin-bottom: 10px;">\u{FFFD} <strong>Section 1 (Text Recognition):</strong> ${section1Score}/5</div>
                <div style="margin-bottom: 10px;">\u{1F4F9} <strong>Section 2 (Video Recognition):</strong> ${section2Score}/5</div>
                <div style="margin-bottom: 10px;">\u{270D}\u{FE0F} <strong>Section 3 (Sign Description):</strong> ${section3Score}/5</div>
            </div>
        `;
        await this.addChatMessage('bot', breakdownHtml);
        await this.delay(1000);
        await this.addChatMessage('bot', `\u{1F50D} Want to dive deeper into specific questions? Use the "<strong>Review Answers</strong>" button above for detailed analysis!`);
        await this.delay(500);
        await this.addChatMessage('bot', `Thanks for taking the Sign Language Quiz! Keep practicing and you'll continue to improve! \u{1F31F}`);
    ...
    delay(ms) {
        if (this.chatSkipAnimations) return Promise.resolve(); // Skip delay when animations are disabled
    ...
    displayReviewQuestion() {
        ...
        if (isSection3) // Section 3 - Text input with AI evaluation
        this.handleSection3Review(answer, sign);
        else // Section 1 & 2 - Multiple choice
    ...
    handleMultipleChoiceReview(answer, sign) {
        ...
        if (this.currentReviewIndex >= 5 && this.currentReviewIndex <= 9) {
            ...
            if (cachedSentence) this.showReviewSentence(word, cachedSentence);
            else this.hideReviewSentence();
        } else this.hideReviewSentence();
    }
    ...
    hideSection3ReviewElements() {
        ...
        if (section3ReviewContainer) section3ReviewContainer.classList.add('hidden');
    }
    hideTraditionalReviewAnswers() {
        const reviewAnswers = document.querySelector('.review-answers');
        if (reviewAnswers) reviewAnswers.classList.add('hidden');
    }
    showTraditionalReviewAnswers() {
        const reviewAnswers = document.querySelector('.review-answers');
        if (reviewAnswers) reviewAnswers.classList.remove('hidden');
    }
    ...
    hideReviewSentence() {
        ...
        if (reviewSentenceContainer) reviewSentenceContainer.style.display = 'none';
    }
}
...
//# sourceMappingURL=quiz.101ca548.js.map
