...
// These are globally available from the drawing_utils.js script loaded in HTML
...
// Prediction result element for the webcam section (if it exists on this page)
const webcamPredictionResultElement = document.getElementById("predictionResult");
...
// Global variables to store the latest detection results from MediaPipe
let handResults: any = undefined;
...
const createDetectors = async () => {
  try {
    ...
    if (webcamPredictionResultElement) {
      webcamPredictionResultElement.innerText = "Prediction: Models loaded, enable webcam for live predictions.";
    }
    ...
    if (webcamPredictionResultElement) {
      webcamPredictionResultElement.innerText = "Prediction: Failed to load ML models. Check console for details.";
    }
  }
};
...
async function predictUploadedVideo() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const predicted_gloss = data.predicted_gloss; // Corrected from data.prediction
      if (videoPredictionResultElement) {
        videoPredictionResultElement.innerText = `Prediction: ${predicted_gloss}`;
      }
    } else {
...
function enableCam(event: Event) {
  ...
  if (webcamRunning === true) {
    ...
    if (webcamPredictionResultElement) { // Use webcam specific element
      webcamPredictionResultElement.innerText = "Prediction: Disabled";
    }
  } else {
    webcamRunning = true;
    ...
    navigator.mediaDevices.getUserMedia(constraints).then((stream) => {
      ...
      if (enableWebcamButton) enableWebcamButton.innerText = "ENABLE PREDICTIONS";
      if (webcamPredictionResultElement) { // Use webcam specific element
        webcamPredictionResultElement.innerText = "Prediction: Webcam access denied or error";
      }
    });
  }
}
...
async function predictWebcam() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const predicted_gloss = data.predicted_gloss; // Corrected from data.prediction
      if (webcamPredictionResultElement) { // Use webcam specific element
        webcamPredictionResultElement.innerText = `Prediction: ${predicted_gloss}`;
      }
    } else {
      console.error('Server error:', response.status, response.statusText);
      if (webcamPredictionResultElement) { // Use webcam specific element
        webcamPredictionResultElement.innerText = `Prediction: Server Error (${response.status})`;
      }
    }
  } catch (error) {
    console.error('Network error during prediction:', error);
    if (webcamPredictionResultElement) { // Use webcam specific element
      webcamPredictionResultElement.innerText = 'Prediction: Network Error (Is server running?)';
    }
  }
