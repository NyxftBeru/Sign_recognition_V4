...
const OPENAI_API_KEY = null; // SECURITY: Do not hardcode API keys - get from user input or environment
...
class SignLanguageQuiz {
    constructor() {
        ...
        // Set OpenAI API key - will be null initially for security
        this.openaiApiKey = OPENAI_API_KEY;
        
        // OpenAI API is disabled by default when no key is provided
        this.openaiDisabled = (OPENAI_API_KEY === null);
    ...
    initializeElements() {
        ...
        this.apiKeySubmit = document.getElementById('api-key-submit');
        this.apiKeyClear = document.getElementById('api-key-clear');
    ...
    bindEvents() {
        ...
        // Debug UI events
        this.debugBtn.addEventListener('click', () => this.openDebugModal());
        this.debugClose.addEventListener('click', () => this.closeDebugModal());
        this.apiKeySubmit.addEventListener('click', () => this.setApiKeyFromUI());
        this.apiKeyClear.addEventListener('click', () => this.clearApiKeyFromUI());
...
2. explain: Please explain the concept to me.`;
    ...
    openDebugModal() {
        ...
        if (this.openaiApiKey && !this.openaiDisabled) {
            this.showDebugStatus('API key is currently set and active', 'success');
        } else {
            this.showDebugStatus('No API key set - OpenAI features disabled', 'error');
        }
    }
    ...
    closeDebugModal() {
        ...
        this.debugStatus.className = 'debug-status';
    }
    ...
    setApiKeyFromUI() {
        ...
        if (!apiKey) {
            this.showDebugStatus('Please enter an API key', 'error');
            return;
        }
        
        if (!apiKey.startsWith('sk-')) {
            this.showDebugStatus('Invalid API key format. OpenAI keys start with "sk-"', 'error');
            return;
        }
        ...
        this.showDebugStatus('✅ API key set successfully! OpenAI features are now enabled.', 'success');
        ...
        console.log('🔑 OpenAI API key set via debug UI');
    }
    ...
    clearApiKeyFromUI() {
        ...
        this.showDebugStatus('API key cleared. OpenAI features disabled.', 'error');
        console.log('🚫 OpenAI API key cleared via debug UI');
    }
    
    showDebugStatus(message, type) {
        this.debugStatus.textContent = message;
        this.debugStatus.className = `debug-status ${type}`;
    }
}
...
- OpenAI API: ${quiz.openaiDisabled ? 'DISABLED' : 'ENABLED'}`);
        ...
        enableOpenAI: () => {
            if (!quiz.openaiApiKey) {
                console.error('❌ No OpenAI API key set. Use window.signQuizDebug.setApiKey("your-key-here") first');
                return;
            }
        ...
        setApiKey: (apiKey) => {
            if (!apiKey || !apiKey.startsWith('sk-')) {
                console.error('❌ Invalid API key format. OpenAI keys start with "sk-"');
                return;
            }
            quiz.openaiApiKey = apiKey;
            quiz.openaiDisabled = false;
            console.log('✅ OpenAI API key set successfully');
            console.log('🚀 OpenAI features are now enabled');
        }
    };
    ...
    console.log('🔑 Use window.signQuizDebug.setApiKey("your-openai-key") to enable AI features');
    console.log('🚫 OpenAI API is currently DISABLED (no API key configured)');
    console.log('🎯 Section 2 (Q6-10) requires OpenAI API key for AI-generated content');
});
